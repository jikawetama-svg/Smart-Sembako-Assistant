...
### Component 0 — Database Alias Learning System (KUNCI UTAMA)

Menggantikan `config.json` hardcode, kita gunakan tabel SQLite agar sistem bisa belajar otomatis.

#### [NEW] Table `ProductAlias` (di `pos.db` atau `bot.db`)
```sql
...
1. Parse nama produk dari struk → `"aci sob"`
2. Cek tabel `ProductAlias` dengan `AliasName` cocok.
3. Jika ada → gunakan `ProductId` langsung (Auto-correct 100%).
4. Jika tidak ada → lakukan Fuzzy Match (Levenshtein) ke seluruh produk.
   - Score > 90% → Auto correct & Simpan Alias.
   - Score 70-90% → Masuk ke mode `NEEDS_REVIEW`, tawarkan 3 opsi ke user via Telegram.
   - Score < 70% → Manual input.

---

### Component 1 — OCR Service & Parsing Engine (BARU)

#### Arsitektur Parsing (Strategy Pattern)
Berdasarkan `remake.md`, kita akan menggunakan pola *Strategy Pattern* agar parser mudah diskalakan per vendor.
...
**[NEW] `Services/Ocr/Interfaces/IInvoiceParser.cs`**
```csharp
...
**[NEW] `Services/Ocr/ParserFactory.cs`**
Factory untuk mendeteksi vendor secara otomatis dan mengembalikan parser yang tepat.
```csharp
public class ParserFactory {
    private readonly IEnumerable<IInvoiceParser> _parsers;
    public ParserFactory(IEnumerable<IInvoiceParser> parsers) {
        _parsers = parsers;
    }
    public IInvoiceParser GetParser(string text) {
        return _parsers.FirstOrDefault(p => p.CanHandle(text)) ?? _parsers.First(p => p is GenericParser);
    }
}
```

**[NEW] `Services/Ocr/OcrReceiptService.cs`**
Service utama yang mengatur *flow*:
1. **`DownloadPhotoAsync(fileId)`** — download foto ke temp file.
2. **`PreprocessImageAsync(imagePath)`** — Menggunakan **ImageMagick** (Grayscale, Resize 2x, Contrast Adjust) untuk mempertajam teks.
3. **`ExtractTextFromImageAsync(imagePath)`** — jalankan Tesseract OCR pada gambar yang sudah diproses.
4. **`ParseReceiptAsync(ocrText)`** — panggil `ParserFactory.GetParser(ocrText)`, jalankan `.Parse()`.
5. **`FuzzyMatchProductsAsync(parsedInvoice)`** — pencocokan nama produk menggunakan tabel `ProductAlias` dan algoritma Levenshtein/Jaro-Winkler.

**Model Internal (`ParsedInvoice.cs`):**
```csharp
public enum OcrItemStatus { VALID, NEEDS_REVIEW }
...
### Component 5 — Telegram UX & State Machine (BARU)

#### [MODIFY] `Services/TelegramBotService.cs`
Implementasikan **State Machine** untuk percakapan OCR yang interaktif menggunakan Inline Keyboard:
- **States:** `IDLE` → `PROCESSING` → `REVIEW` → `EDIT_ITEM` → `CONFIRM` → `DONE`
- `HandleUpdateAsync()`: Jika kirim foto dengan caption `/struk`, masuk state `PROCESSING`.
- Bot memproses dan membalas dengan **Summary** (Berapa VALID, berapa NEEDS_REVIEW).
- Tombol: `[1. Simpan Sekarang]` atau `[2. Review Item]`.
- Jika `Review Item` diklik, kirim daftar item yang `NEEDS_REVIEW`.
- Jika user memilih perbaikan (klik suggestion 1, 2, atau 3), **simpan ke `ProductAlias`** agar AI belajar.
- Terapkan **Hybrid Tolerance**: User bisa klik "Simpan Sekarang" meskipun ada error. Data valid disimpan ke Aronium, data error masuk tabel/antrean *Retry Queue*.
...
2. HandleOcrPhotoAsync()
   → Download foto
   → ImageMagick Pre-processing (Grayscale, Resize)
   → OcrReceiptService.ExtractTextFromImageAsync() → raw text
   → OcrReceiptService.ParseReceiptLines() → List<ParsedItem>
   → OcrReceiptService.FuzzyMatchProductsAsync() → match ke DB & ProductAlias
   → SaveSessionData(state="REVIEW", data=JSON items)
   → Kirim pesan Summary + Inline Keyboards [Simpan] [Review]
```

### Saat Konfirmasi Simpan (Hybrid Tolerance)
```
3. ConfirmActionAsync()
   → loop items: pisahkan yang Status == VALID dan NEEDS_REVIEW
   → PosDbService.CreateBulkPurchaseDocumentAsync(validItems)
      ↳ SATU Document header (Type 1)
      ↳ N DocumentItem (hanya yg valid)
      ↳ UPDATE Stock
      ↳ COMMIT
   → GoogleSheetsService.AppendPurchaseRowsAsync(validItems, docNumber)
   → Jika ada NEEDS_REVIEW, simpan ke antrean Retry Queue di DB lokal.
   → Kirim pesan Sukses ke Telegram.
```

---

## Preview Message Format (State: REVIEW)

```text
🧾 Struk terdeteksi (WINGS FOOD)
✅ 4 item valid
⚠️ 2 item perlu dicek

Total Terbaca: Rp 1.167.010

Pilih aksi:
[1. Simpan Sekarang (Simpan 4 item, Antrikan 2)]
[2. Review Item Error]
[3. Batal]
```

*(Jika klik 2. Review Item Error)*:
```text
⚠️ Item 1 bermasalah:
"Potabee Reg BBQ" -> tidak dikenali

Mungkin maksudnya:
[1. Potabee BBQ 68gr]
[2. Potabee Seaweed 68gr]
[3. Ketik Manual]
[4. Abaikan (Skip)]
```
